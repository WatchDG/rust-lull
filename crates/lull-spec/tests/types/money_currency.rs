...
use lull_spec::types::{CurrencyCode, CurrencyId, MoneyCurrency};

type TestMoneyCurrency = MoneyCurrency<CurrencyRef<String, [u8; 3]>>;

#[test]
fn equal_currencies_are_equal() {
    let left: TestMoneyCurrency =
        MoneyCurrency::new(CurrencyRef::Id(CurrencyId::new(String::from("cur-1"))));
...
fn distinct_currencies_are_not_equal() {
    let left: TestMoneyCurrency =
        MoneyCurrency::new(CurrencyRef::Id(CurrencyId::new(String::from("cur-1"))));
    let right: TestMoneyCurrency =
        MoneyCurrency::new(CurrencyRef::Id(CurrencyId::new(String::from("cur-2"))));
...
fn equal_currencies_hash_to_the_same_bucket() {
    ...
    currencies.insert(MoneyCurrency::new(CurrencyRef::Id(CurrencyId::new(
        String::from("cur-1"),
    ))));
    currencies.insert(MoneyCurrency::new(CurrencyRef::Id(CurrencyId::new(
        String::from("cur-1"),
    ))));
    currencies.insert(MoneyCurrency::new(CurrencyRef::Code(CurrencyCode::new(
        *b"USD",
    ))));
    currencies.insert(MoneyCurrency::new(CurrencyRef::Code(CurrencyCode::new(
        *b"USD",
