...
fn node(id: &'static str) -> Node<&'static str, &'static str, &'static str, ()> {
    Node::new(NodeId::new(id), "RiskEngine", "var_limit", ())
}

#[test]
fn builder_returns_core_dag() {
    let dag = CoreDAGBuilder::<_, _, _, _, (), ()>::new()
        .node(node("a"))
        .node(node("b"))
...
fn builder_rejects_a_cycle() {
    let result = CoreDAGBuilder::<_, _, _, _, (), ()>::new()
        .node(node("a"))
        .node(node("b"))
        .edge(Edge::new(NodeId::new("a"), NodeId::new("b")))
